    ...
    for rii=1:length(ris);
        ...
        if exist([fl '.mat'])>0;
            ...
            if size(gdata,1)>10 & size(data,1)>10;
                ...
                listener_cord=reshape(listener_cord',size(speaker_cord,2),subjN,binN);
                listener_cord=permute(listener_cord,[3 1 2]);
                
                save([expdir '/' exp '/fmri/' rname '_tr_pattern_binSize' num2str(binSize) '_cmds_' type '_' col_speaker '.mat'],'speaker_cord','listener_cord','binN','binSize','binStep','rname','d','Y')
