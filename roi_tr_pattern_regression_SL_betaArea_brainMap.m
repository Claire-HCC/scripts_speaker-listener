...
for ei=[1:4];;%
    ...
    save([expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/SLeach/regression_SLeach_lag' num2str(min(lags)) '-' num2str(max(lags)) '_bLagArea'  ],'rnames','blag_pos','blag_neg');
    
    nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( blag_neg)),   blag_neg(~isnan(  blag_neg))]);
    save_nii(nii,[expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/SLeach/regression_SL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_blag_neg.nii']);
    nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( blag_pos)),   blag_pos(~isnan(  blag_pos))]);
    save_nii(nii,[expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/SLeach/regression_SL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_blag_pos.nii']);
end
