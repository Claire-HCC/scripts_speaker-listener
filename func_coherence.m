...
function [ Pxx, Pyy, Pxy, coh, pha, freq ] = func_coherence( f1, f2, win,noverlap,Fs )
% Claire replaced csd with cpsd to get the same results as mscohere
...
% [Pxy,freq] = csd( f1, f2, nfft, Fs, filt, n_overlap );
 ...
 [Pxx,freq]=cpsd(f1,f2,win,noverlap,[],Fs);
 [Pyy,freq]=cpsd(f2,f2,win,noverlap,[],Fs);
 [Pxy,freq]=cpsd(f1,f2,win,noverlap,[],Fs);
Kxy  = real( Pxy );
Qxy  = imag( Pxy );
