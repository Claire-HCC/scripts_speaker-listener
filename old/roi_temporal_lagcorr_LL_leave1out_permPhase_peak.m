...
for ei=1:4;
    ...
    for lagi=1:length(lags_tested);
        ...
        load([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2roi/' froidir '/LL_leave1out/perm/lag' num2str(min(lags)) '-' num2str(max(lags)) '_permPhase' ],'rnames','r','lags','keptT');
        ...
        nii.img(1,1,1)=1;
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2roi/'  froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags))  '_posPeakLags_pfwe.nii']);
        
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( peakLags_pfwe) & peaks_pfwe<0),   peakLags_pfwe(~isnan( peaks_pfwe) & peaks_pfwe<0)+0.00000001]);
        nii.img(1,1,1)=1;
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2roi/'  froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags))  '_negPeakLags_pfwe.nii']);
        ...
        nii.img(1,1,1)=1;
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2roi/'  froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags))  '_posPeakLags_pfdr.nii']);
        
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( peakLags_pfdr) & peaks_pfdr<0),   peakLags_pfdr(~isnan( peaks_pfdr) & peaks_pfdr<0)+0.00000001]);
        nii.img(1,1,1)=1;
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2roi/'  froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags))  '_negPeakLags_pfdr.nii']);
        
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( peaks_pfwe) ),   peaks_pfwe(~isnan( peaks_pfwe) )]);
        nii.img(1,1,1)=1;
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2roi/'  froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags))  '_peaks_pfwe.nii']);
        
        save([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2roi/' froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags)) '_peaks' ],...
