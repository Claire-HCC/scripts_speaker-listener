...
for ei=12;%[1 2 4 9:12]
    ...
    for bi=[1]%:2;
        ...
        for lagi=1%:length(lags_tested);
            ...
            load([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_leave1out_bined/' networks{1} '_binSize' num2str(binSize) '_lag' num2str(min(lags)) '-' num2str(max(lags)) ],'networks','r','lags','keptT');
            [~,tn,~,listenerN]=size(r);
            rz=nan([length(networks) length(networks) tn length(lags) listenerN]);
            
            for sdi=1:length(networks);
                seed=networks{sdi};
                if exist([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_leave1out_bined/' seed '_binSize' num2str(binSize)  '_lag' num2str(min(lags)) '-' num2str(max(lags)) '.mat']);
                    load([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_leave1out_bined/' seed  '_binSize' num2str(binSize)  '_lag' num2str(min(lags)) '-' num2str(max(lags)) ],'networks','r','lags','keptT');
                    rz(sdi,:,:,:,:)=atanh(r);
                end
            end
            ...
            save([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_leave1out_bined/networks2networks' '_binSize' num2str(binSize)  '_lag' num2str(min(lags)) '-' num2str(max(lags)) '_groupT2tail_peaks' ],...
