function roi_tr_pattern_regression_SLeach_betaClassification(ei)
...
%     load(sprintf('%s/%s/fmri/pattern_regression/%s/roi/%s/SLeach/perm/regression_SLeach_lag%d-%d_permSL%03d',expdir,exp,timeUnit,froidir,min(lags),max(lags),0),'b');
%     
%     b_s=b(:,2:end,:);
%     subjN=size(b_s,3);
...
%         load(sprintf('%s/%s/fmri/pattern_regression/%s/roi/%s/SLeach/perm/regression_SLeach_lag%d-%d_permSL%03d',expdir,exp,timeUnit,froidir,min(lags),max(lags),perm),'b');
...
%         end
%     end
      load([expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/SLeach/regression_SLeach_lag' num2str(min(lags)) '-' num2str(max(lags)) '_classification' ],'rnames','acc','b_s','b_l','foldN');
    ...
    save([expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/SLeach/regression_SLeach_lag' num2str(min(lags)) '-' num2str(max(lags)) '_classification' ],'rnames','acc','sig_fdr_withinSigSL','b_s','b_l','p','p_adj','foldN','p_adj_withinSigSL');
end
