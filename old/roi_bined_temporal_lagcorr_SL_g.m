function roi_bined_temporal_lagcorr_SL_g(ei)
...
exp=experiments{ei};
mkdir([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/' froidir '/SL_g_bined/perm/']);
...
for binSizei=1%:length(binSize_tested);
    ...
    for lagi=1;%1:length(lags_tested);
        ...
        r=nan([length(rnames) tn  length(lags) permN]);
        ...
        save([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/' froidir '/SL_g_bined/binSize' num2str(binSize) '_lag' num2str(min(lags)) '-' num2str(max(lags)) ],'r','rnames','binSize','lags');
        clear r
    end
end
