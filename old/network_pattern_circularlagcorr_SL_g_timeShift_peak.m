...
for ei=1:2;%
    ...
    for lagi=3;%1:length(lags_tested);
        ...
        f=([expdir '/' exp '/fmri/pattern/circularlagcorr/' timeUnit '/network/' froidir '/SL_g/r.mat']);
        load(f,'r','keptT');
        rz=atanh(r);
        ...
        save([expdir '/' exp '/fmri/pattern/circularlagcorr/' timeUnit '/network/' froidir '/SL_g/network_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeShift_peaksMax' ],...
