...
for ei=13;
    ...
    for lagi=1:length(lags_tested);
        ...
        for sdi=1:length(networks);
            ...
            load([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_leave1out/' f ],'networks','r','lags','keptT');
            rz(sdi,:,:,:)=atanh(r);
        end
        ...
        for sdi=1:length(networks);
            for ni=1:length(networks);
                ...
                if ~isempty(pks);
                    ...
                    % locs(~ismember(locs,find(squeeze(pfwe(sdi,ni,:)<0.05))))=[];
                    % if ~isempty(locs);
                    %    [~,loci]=(min(abs(locs-find(lags==0))));
        ...
        save([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_leave1out/networks2networks_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeShift_peaksNearest' ],...
            'networks','rzm','rz','lags','keptT','p','pfwe','peakLags','peaks','peaks_pfwe','peakLags_pfwe','peaks_p','peakLags_p');
    end
end
