...
for ei=[1 2 4 9:12];%[1 2 4 11 12]
    exp=experiments{ei};
    
    for lagi=1%:length(lags_tested);
        lags=lags_tested{lagi};
        
        load([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/network2networks_peakLag0_mask/' froidir '/LL_leave1out/' networks{1} '_lag' num2str(min(lags)) '-' num2str(max(lags)) ],'networks','r','lags','keptT');
        [~,~,listenerN]=size(r);
        ...
        save([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/network2networks_peakLag0_mask/' froidir '/LL_leave1out/networks2networks_lag' num2str(min(lags)) '-' num2str(max(lags)) '_groupT2tail_peaks' ],...
