function roi_tr_temporal_regression_SLeach_betaClassification(ei)
...
for lagi=1%:length(lags_tested);
    ...
    load(sprintf('%s/%s/fmri/temporal_regression/%s/roi/%s/SLeach/regression_SLeach_lag%d-%d',expdir,exp,timeUnit,froidir,min(lags),max(lags)),'b');
    ...
    save([expdir '/' exp '/fmri/temporal_regression/' timeUnit '/roi/' froidir '/SLeach/regression_SLeach_lag' num2str(min(lags)) '-' num2str(max(lags)) '_classification' ],'rnames','acc','b_s','b_l','p','p_adj','foldN');
end
