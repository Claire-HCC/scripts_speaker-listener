...
for ei=1:4;
    ...
    for lagi=1%:length(lags_tested);
        ...
        load([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/' froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags)) ],'rnames','r','lags','keptT');
        rz=atanh(r);
        rzm=nanmean(rz,3);
        p=nan(size(rzm));
        t=p;
        ...
        nii.img(1,1,[1 2])=[-1 1];
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/'  froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags))  '_groupT_posPeakLags_pfwe.nii']);
        ...
        nii.img(1,1,[1 2])=[-1 1];
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/'  froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags))  '_groupT_posPeakLags_pfdr.nii']);
        
        save([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/' froidir '/LL_leave1out/lag' num2str(min(lags)) '-' num2str(max(lags)) '_groupT_peaks' ],...
