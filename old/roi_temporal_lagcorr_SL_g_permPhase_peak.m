...
for ei=4;%1:4;
    ...
    for lagi=1:length(lags_tested);
        ...
        load([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/' froidir '/SL_g/perm/lag' num2str(min(lags)) '-' num2str(max(lags)) '_permPhase' ],'rnames','r','lags','keptT');
        rz_perm=atanh(r);
        
        load([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/' froidir '/SL_g/lag' num2str(min(lags)) '-' num2str(max(lags)) ],'rnames','r','lags','keptT');
        rz=atanh(r);
        ...
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( peakLags_pfwe) & peaks_pfwe<0),   peakLags_pfwe(~isnan( peaks_pfwe) & peaks_pfwe<0)]);
        nii.img(1,1,[1 2])=[-1 1];
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/'  froidir '/SL_g/lag' num2str(min(lags)) '-' num2str(max(lags))  '_negPeakLags_pfwe.nii']);
        ...
        nii.img(1,1,[1 2])=[-1 1];
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/'  froidir '/SL_g/lag' num2str(min(lags)) '-' num2str(max(lags))  '_posPeakLags_orders_pfdr.nii']);
        ...
        nii.img(1,1,[1 2])=[-1 1];
        save_nii(nii,[expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi/'  froidir '/SL_g/lag' num2str(min(lags)) '-' num2str(max(lags))  '_peaks_pfwe.nii']);
