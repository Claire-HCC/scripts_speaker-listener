function roi_temporal_regression_SL_g(ei)
...
crop_start=10;
lags_tested={-4:4,-10:-4. 4:10,0 ,-10:10, -40:40};

exp=experiments{ei};
% rmdir([expdir '/' exp '/fmri/temporal/regression/' timeUnit '/roi/' froidir '/SL_g/'],'s');
% mkdir([expdir '/' exp '/fmri/temporal/regression/' timeUnit '/roi/' froidir '/SL_g/perm']);
...
for lagi=1%:length(lags_tested);
    ...
    save([expdir '/' exp '/fmri/temporal/regression/' timeUnit '/roi/' froidir '/SL_g/lag' num2str(min(lags)) '-' num2str(max(lags)) ],'b','F','r2','p','lags','rnames','keptT','r2_byTime');
