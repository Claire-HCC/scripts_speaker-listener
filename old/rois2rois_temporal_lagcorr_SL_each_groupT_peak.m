...
for ei=[1:4]
    ...
    for lagi=1%:length(lags_tested);
        ...
        load([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2rois/' froidir '/SL_each/' rnames{1} '_lag' num2str(min(lags)) '-' num2str(max(lags)) ],'rnames','r','lags','keptT');
        [~,~,listenerN]=size(r);
        rz=nan([length(rnames) length(rnames) length(lags) listenerN]);
        
        for sdi=1:length(rnames);
            seed=rnames{sdi};
            if exist([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2rois/' froidir '/SL_each/' seed '_lag' num2str(min(lags)) '-' num2str(max(lags)) '.mat']);
        ...
        for sdi=1:length(rnames);
            ...
            save([expdir '/' exp '/fmri/temporal/lagcorr/' timeUnit '/roi2rois/' froidir '/SL_each/rois2rois_lag' num2str(min(lags)) '-' num2str(max(lags)) '_groupT_peaks' ],...
