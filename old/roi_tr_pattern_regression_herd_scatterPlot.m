...
for ei=3%:4;
    exp=experiments{ei};
    load([expdir '/' exp '/fmri/pattern_regression_bined/' timeUnit '/roi/' froidir '/regression_LL_lag0-0_bined'],'couplingz','keptT','rnames');
    cp_ll=nanmean(couplingz,3);
    cp_ll=cp_ll(:,keptT);
    
    load([expdir '/' exp '/fmri/pattern_regression_bined/' timeUnit '/roi/' froidir '/regression_SL_lag' num2str(min(lags)) '-' num2str(max(lags)) '_bined'],'couplingz','keptT');
    cp_sl=couplingz;
    cp_sl=cp_sl(:,keptT);
