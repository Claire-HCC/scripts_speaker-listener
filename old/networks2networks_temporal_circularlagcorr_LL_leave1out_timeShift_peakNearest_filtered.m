...
for ei=[9 10 13];
    ...
    for lagi=3;%1:length(lags_tested);
        ...
        for fi=1:length(filtertypes);
            ...
            save([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_leave1out/networks2networks_' filtertype '_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeShift_peaksNearest' ],...
