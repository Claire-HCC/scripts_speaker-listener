...
for ei=1:4;
    exp=experiments{ei};
        ...
        load([expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/herd_' num2str(min(lags)) '-' num2str(max(lags)) ],'herd_sig');
    ...
    b_sig=zeros(size(p));
    p_temp=p(couple_sig_fdr==1 & herd_sig==1,:);
    b_sig_sl=zeros(size(p));
    
    % find significant betas withing regions showing significant sl
    % coupling
    b_sig_sl(couple_sig_fdr==1 & herd_sig==1,:)=reshape(fdr0(p_temp(:),0.05),sum(couple_sig_fdr),size(p,2));
    [bSig_sl_ri bSig_sl_lagi]=find(b_sig_sl);
    
    figure(figscatter);
    subplot(2,4,ei+4);
    % focus on DMN network
    histogram(lags(bSig_sl_lagi),round(length(lags)/2),'FaceColor',[0.5 0.5 0.5]);
    ...
    b_sig_ll(couple_sig_fdr==1 & herd_ig==1,:)=reshape(fdr0(p_temp(:),0.05),sum(couple_sig_fdr),size(p,2));
    [bSig_ll_ri bSig_ll_lagi]=find(b_sig_ll);
    
    subplot(2,4,ei);
    % focus on DMN network
    histogram(lags(bSig_ll_lagi),round(length(lags)/2),'FaceColor',[0.5 0.5 0.5]);
    ylabel('Frequency');
