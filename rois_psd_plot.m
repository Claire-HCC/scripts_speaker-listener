...
for ei=1:10;
    ...
    load([expdir '/' exp '/fmri/temporal/frequency/' timeUnit '/roi/' froidir '/L_g/rois_psd'  ],'Sxx','rnames','keptT','freq');
    nansum(Sxx,2)'
    ...
    for ri=1:length(rnames);
        plot(log(freq(2:end)),Sxx(ri,2:end),'color',cols(ri,:),'linewidth',3);
        %    semilogx(freq(2:end),Sxx(ri,2:end),'color',cols(ri,:),'linewidth',2);
        hold on
    end
