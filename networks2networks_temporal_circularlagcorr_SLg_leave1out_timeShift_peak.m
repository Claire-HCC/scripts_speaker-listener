...
for ei=[1 2];%[1 2 4 11 12  13]
    ...
    for lagi=4%:length(lags_tested);
        ...
        for sdi=1:length(networks);
            seed=networks{sdi};
            f=ls([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/SLg/' seed '.mat']);
            load([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/SLg/' f ],'networks','r','lags','keptT');
            rz(sdi,:,:)=atanh(r);
        ...
        save([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/SLg/networks2networks_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeShift_peaksMax' ],...
