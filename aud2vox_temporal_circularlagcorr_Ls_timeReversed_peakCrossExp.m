...
for ei=1:length(eis);
    ...
    load([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/aud2vox/'   '/Ls/aud_timeReversed.mat'   ],'r','keptT');
...
save([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/aud2vox/'   '/Ls/aud2voxIsc' num2str(perc*100) 'PercMasked_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeReverse_fdr_peakLags.mat' ],...
    '','rzms','rzms_timeReversed','rzmsm','rzmsm_timeReversed','lags','peakLags','peaks','rzm','pfdr_peaks','pfdr_exps_peaks','pfdr_time_peaks','tmid');
...
mat(keptvox)=sig;
nii=mat2nii(mat);
save_nii(nii,[expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/aud2vox/'  '/Ls/aud2voxIsc' num2str(perc*100) 'PercMasked_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeReversed_pfdr_mask.nii' ]);
...
mat(keptvox)=peaks;
nii=mat2nii(mat);
save_nii(nii,[expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/aud2vox/'  '/Ls/aud2voxIsc' num2str(perc*100) 'PercMasked_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeReversed_peaks.nii' ]);


mat=nan(voxn,1);
mat(keptvox)=peakLags;
nii=mat2nii(mat);
save_nii(nii,[expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/aud2vox/'  '/Ls/aud2voxIsc' num2str(perc*100) 'PercMasked_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeReverse_peakLags.nii' ])
...
nii=mat2nii(mat);
save_nii(nii,[expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/aud2vox/'  '/Ls/aud2voxIsc' num2str(perc*100) 'PercMasked_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeReversed_peakLagsBined.nii' ]);
