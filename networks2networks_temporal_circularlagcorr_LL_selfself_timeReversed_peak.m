...
for ei=1:10;
    ...
    for lagi=3%:length(lags_tested);
        ...
        for sdi=1:length(networks);
            ...
            load([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_selfself/' seed ],'r','keptT');
            rz(sdi,:,:,:)=atanh(r-0.0000001);
            
            load([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/network2networks/' froidir '/LL_selfself/' seed '_timeReversed' ],'r','keptT');
            rz_timeReversed(sdi,:,:,:)=atanh(r-0.0000001);
        end
        rzm=nanmean(rz,4);
