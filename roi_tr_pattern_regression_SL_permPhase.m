function roi_tr_pattern_regression_SL_permPhase(perm)
...
for ei=1:4;
    ...
    save(sprintf('%s/%s/fmri/pattern_regression/%s/roi/%s/SLg/perm/regression_SL_lag%d-%d_permPhase%04d',expdir,exp,timeUnit,froidir,min(lags),max(lags),perm),'b','r2','lags','rnames','keptT');
    clear b r2 keptT rp
end
