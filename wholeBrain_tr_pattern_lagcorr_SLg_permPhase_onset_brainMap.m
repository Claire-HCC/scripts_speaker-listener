...
for ei=1:4;%[1 2 4];%2:4;
    ...
    for lagi=1;%1:length(lags_tested);
        ...
        load(sprintf('%s/%s/fmri/temporal_lagcorr/%s/wholeBrain/SLg/perm/lag%d-%d_permPhase',expdir,exp,timeUnit,min(lags),max(lags)),'r');
        r_perm=r;
        ...
        mat(keptvox)=onsets;
        nii=mat2nii(mat);
        save_nii(nii,[expdir '/' exp '/fmri/temporal_lagcorr/' timeUnit '/wholeBrain/SLg/SL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_onsets.nii']);
        %
        %         nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( onsetsOrder)),   onsetsOrder(~isnan( onsetsOrder))]);
        %         save_nii(nii,[expdir '/' exp '/fmvi/temporal_lagcorr/' timeUnit '/wholeBrain/' froidir '/SLg/SL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_onsetsOrder.nii']);
        %
        %         save([expdir '/' exp '/fmvi/temporal_lagcorr/' timeUnit '/wholeBrain/' froidir '/SLg/SL_lag' num2str(min(lags)) '-' num2str(max(lags)) '_onsets' ],'keptT','r','lags','keptT','r_perm','p','p_fdr','onsets','onsetsOrder');
    end
end
