...
for ei=1:9;
    ...
    for ri=1:1:length(rois);
        ...
        for si=1:listenerN;
            ...
            y=zscore(gdata_target(:,keptT,si),0,2)';
            x=nanmean(zscore(gdata_seed(:,keptT,si),0,2),3)';
            x=repmat(fliplr(x),1,size(y,2));
            
            r(:,:,si)=circularlagcorr_byCol(x,y,lags)';
        end
        
        save([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/roi2vox/' froidir '/LL_selfself/' roi '_timeReversed.mat'   ],'r','lags','keptvox','keptT','-v7.3');
    end
end
