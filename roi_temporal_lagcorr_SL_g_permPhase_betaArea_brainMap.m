...
for ei=3;%2:4;
    ...
    for lagi=1:length(lags_tested);
        ...
        save_nii(nii,[expdir '/' exp '/fmri/temporal_lagcorr/' timeUnit '/roi/' froidir '/SLg/SL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_betaArea_neg.nii']);
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( lag_pos)),   lag_pos(~isnan(  lag_pos))]);
        save_nii(nii,[expdir '/' exp '/fmri/temporal_lagcorr/' timeUnit '/roi/' froidir '/SLg/SL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_betaArea_pos.nii']);
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( lag_both)),   lag_both(~isnan(  lag_both))]);
        save_nii(nii,[expdir '/' exp '/fmri/temporal_lagcorr/' timeUnit '/roi/' froidir '/SLg/SL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_betaArea_both.nii']);
        
        save([expdir '/' exp '/fmri/temporal_lagcorr/' timeUnit '/roi/' froidir '/SLg/SL_lag' num2str(min(lags)) '-' num2str(max(lags)) '_stats' ],'rnames','r','lags','keptT','r_perm','p','lag_pos','lag_neg','lag_both');
    end
end
