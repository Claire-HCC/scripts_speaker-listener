...
for ei=1;
    ...
    for ci=1:max(b_cluster(:));
        roi_table_inds=cellfun(@(x) strmatch(x,roi_table.region,'exact'),rnames(b_cluster==ci));
        roi_ids=cell2mat(roi_table.id(roi_table_inds));
        nii=roiTable2wholeBrainNii_mor([roi_ids, ci*ones(length(roi_ids),1)]);
        save_nii(nii,[expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/herd_cluster' num2str(ci) '.nii']);
    end
    
    for ci=1:max(b_cluster);
        bz_byHerdCluster(ci,:)=mean(b_sl_z(b_cluster==ci,:),1);
        figure;
        plot(lags*tr(ei),mean(b_sl_z(b_cluster==ci,:),1),'k','linewidth',2);
        xlabel(['speaker precedes      shift (sec)    listener precedes']);
        ylabel('mean weigths');
        set(gca,'fontsize',14)
        grid on
        title([exp ': cluster' num2str(ci)]);
        print(gcf,[expdir '/' exp '/fmri/pattern_regression/' timeUnit '/roi/' froidir '/herd_cluster' num2str(ci) '.tif'],'-dtiff');
