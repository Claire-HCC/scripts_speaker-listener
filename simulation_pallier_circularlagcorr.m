...
for fi=1:length(filelist);
    if exist(sprintf('%s/simulation/sim%03d/%s',expdir,simN ,[strrep(filelist{fi},'.mat','') '_r.mat']))==0;
        ...
        for simi=1:simN;
            for sdi=1:levn;
                for tgi=1:levn;
                    r(sdi,tgi,:,simi)=circularlagcorr(v_hrf_tr(sdi,:,simi),v_hrf_tr(tgi,:,simi),-floor((tn-1)/2):floor((tn-1)/2));
                    r_timeReversed(sdi,tgi,:,simi)=circularlagcorr(v_hrf_tr(sdi,:,simi),fliplr(v_hrf_tr(tgi,:,simi)),-floor((tn-1)/2):floor((tn-1)/2));
                end
            end
        end
        
        save(sprintf('%s/simulation/sim%03d/%s',expdir,simN ,[strrep(filelist{fi},'.mat','') '_r.mat']),...
