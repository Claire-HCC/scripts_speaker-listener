...
for ei=1:4;%1:4;%[1 2 4];%2:4;
    ...
    for lagi=1;%1:length(lags_tested);
        ...
        % r-nanmean(r_perm,3);
        peak2=nan([length(rnames) 1 ]);
        peakLags2=peak2;
        r_temp=r-nanmean(r_perm,3);
        r_temp(:,lags<0)=NaN;
        ris=find(sum(~isnan(r_temp),2)~=0);
        [peak2(ris) peakLagi_pfdr]=max(r_temp(ris,:),[],2);
        peakLags2(ris,1)=(lags(peakLagi_pfdr))';
        
        
        peak2_pfdr=nan([length(rnames) 1 ]);
        peakLags2_pfdr=peak2_pfdr;
        r_temp(p_fdr>.05)=NaN;
        ris=find(sum(~isnan(r_temp),2)~=0);
        [peak2_pfdr(ris) peakLagi_pfdr]=max(r_temp(ris,:),[],2);
        peakLags2_pfdr(ris,1)=(lags(peakLagi_pfdr))';
        
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( peak_pfdr)),   peak_pfdr(~isnan( peak_pfdr))]);
        save_nii(nii,[expdir '/' exp '/fmri/pattern_lagcorr/' timeUnit '/roi/'  froidir '/SLg/SL_permSL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_peak_pfdr.nii']);
        
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( peakLags)),   peakLags(~isnan(peakLags))+0.00000001]);
        nii.img(1,1,1)=1;
        save_nii(nii,[expdir '/' exp '/fmri/pattern_lagcorr/' timeUnit '/roi/'  froidir '/SLg/SL_permSL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_peakLags.nii']);
        
        nii=roiTable2wholeBrainNii_mor([roi_ids(~isnan( peakLags_pfdr)),   peakLags_pfdr(~isnan( peakLags_pfdr))+0.00000001]);
        nii.img(1,1,1)=1;
        save_nii(nii,[expdir '/' exp '/fmri/pattern_lagcorr/' timeUnit '/roi/'  froidir '/SLg/SL_permSL_lag' num2str(min(lags)) '-' num2str(max(lags))  '_peakLags_pfdr.nii']);
        
        save([expdir '/' exp '/fmri/pattern_lagcorr/' timeUnit '/roi/' froidir '/SLg/SL_permSL_lag' num2str(min(lags)) '-' num2str(max(lags)) '_peaks' ],'rnames','r','lags','keptT','r_perm','p','p_fdr','peak','peakLags','peakLags_pfdr','peak_pfdr','peakLags2_pfdr','peak2_pfdr','peakLags2','peak2');
    end
end
