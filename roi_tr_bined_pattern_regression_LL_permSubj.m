...
for binSizei=1:length(binSize_tested);
    ...
    for ei=[1 2 4];
        ...
        save([expdir '/' exp '/fmri/pattern_regression_bined/' timeUnit '/roi/' froidir '/regression_LL_binSize' num2str(binSize) '_lag' num2str(min(lags)) '-' num2str(max(lags))  '_perm' num2str(perm)],'b','F','r2','p','lags','rnames','-v7.3');
    end
end
