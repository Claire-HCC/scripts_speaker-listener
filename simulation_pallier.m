function simulation_pallier(us,vars,speechRs,NRFs,pauseLens,pauseEffects);
% us=3; % 2:4; 3000 words are often not enough to create level 6 with more than to 2 consituents.
% vars=0.5;
% speechRs=1;
% NRFs={'linear'}; % {'linear', 'log','expn', 'triangle','linearR','boxcar','time'};
% pauseLens=3;
% pauseEffects=0.1; % activity at pause period = min acitivity-sd of activity *pauseEffect
% simulation_pallier(2:4,vars,speechRs,NRFs,pauseLens,pauseEffects);
% simulation_pallier(us,0:0.5:1,speechRs,NRFs,pauseLens,pauseEffects);
% simulation_pallier(us,vars,0.5:0.5:1.5,NRFs,pauseLens,pauseEffects);
% simulation_pallier(us,vars,speechRs,{'linear', 'log','expn', 'triangle','time'},pauseLens,pauseEffects);
% simulation_pallier(us,vars,speechRs,NRFs,[0 1.5 3 4.5 5],pauseEffects);
% simulation_pallier(us,vars,speechRs,NRFs,pauseLens,[0:2]);
            ...
            for nrfi=1:length(NRFs);
                ...
                for pauseLeni=1:length(pauseLens);
                    ...
                    for pauseEffecti=1:length(pauseEffects);
                        ...
                        if exist(sprintf('%s/simulation/sim%03d/pause%.2f_pauseL%.1f_m%d_v%.2f_sr%.2f_%s.mat',expdir,simN,pauseEffect,pauseLen,u,var,speechR, NRF))==0;
                            try
                                for simi=1:simN;
                                    ...
                                    for lev=3:(levn+1);
                                        ...
                                        constituentL_sample=constituentL_sample(1:n_current);
                                        onsets_current=onsets_below(cumsum(constituentL_sample));
                                    ...
                                    for ci=1:(length(onsets));
                                        ...
                                        if strcmp(NRF,'boxcar'); % pauseEffect does not matter with boxcar NRF
                                            pause_temp=zeros(size(pause_temp));
                                        end
                                ...
                                if simN==1;
                                    save(sprintf('%s/simulation/sim%03d/pause%.2f_pauseL%.1f_m%d_v%.2f_sr%.2f_%s.mat',expdir,simN,pauseEffect,pauseLen,u,var,speechR, NRF),'v_hrf_tr','v_tr','v','v_hrf','onsetsVectors');
                                else
                                    save(sprintf('%s/simulation/sim%03d/pause%.2f_pauseL%.1f_m%d_v%.2f_sr%.2f_%s.mat',expdir,simN,pauseEffect,pauseLen,u,var,speechR, NRF),'v_hrf_tr','v_tr');
                                end
                            catch
                                continue;
                            end
