...
for ei=2;% [4 11 9 10];
    ...
    mkdir([expdir '/' exp '/fmri/timeseries/' timeUnit '/network/' froidir '_audPauseResid/']);
    ...
    load([expdir exp '/sound/onsets.mat'],'offsets_tr_wd','offsets_tr_sn','offsets_tr_pr');
    ...
    X_temp=zeros(tn,3);
    X_temp(round(offsets_tr_wd),1)=1;
    X_temp(round(offsets_tr_sn),2)=1;
    X_temp(round(offsets_tr_pr),3)=1;
    
    %     temp=[offsetsVector_wd;
    %         offsetsVector_sn;
    %         offsetsVector_pr]';
    ...
    for sdi=1:length(networks);
        ...
        mkdir([expdir '/' exp '/fmri/timeseries/' timeUnit '/network/' froidir '_audPausesResid/']);
        save([expdir '/' exp '/fmri/timeseries/' timeUnit '/network/' froidir '_audPausesResid/listenerAll_' seed '.mat' ],'gdata','keptvox','Y','y');
    end
end
