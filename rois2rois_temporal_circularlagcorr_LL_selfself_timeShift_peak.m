...
for ei=[1 13 2 4 9:12];
    ...
    for lagi=3;%1:length(lags_tested);
        ...
        for sdi=1:length(rnames);
            seed=rnames{sdi};
            f=([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/roi2rois/' froidir '/LL_selfself/' seed '.mat']);
            if exist(f);
                load(f,'r','keptT');
                if sdi==1;
                    rz=nan(size(r,1),size(r,1),size(r,2),size(r,3));
                end
                rz(sdi,:,:,:)=atanh(r-0.00001);
            end
        end
        rzm=nanmean(rz,4);
        [~,~,tn]=size(rzm);
        ...
        save([expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/roi2rois/' froidir '/LL_selfself/rois2rois_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeShift_peaksMax' ],...
   ...
   temp=(widths_pfwe(eye(length(rnames))==1));
    ...
    save_nii(nii,[expdir '/' exp '/fmri/temporal/circularlagcorr/' timeUnit '/roi2rois/' froidir '/LL_selfself/rois2rois_lag' num2str(min(lags)) '-' num2str(max(lags)) '_timeShiftPfwe_acf.nii']);
